...
export default function DiaryWriteScreen() {
        ...
        [
          {
            ...
            onPress: () => {
              setContent('');
              setRecordedURI(null);
              setCurrentDiaryId(null); // 새로 작성하므로 현재 ID 초기화
            },
            style: "cancel"
          },
          {
            text: "이어서 작성",
            onPress: () => loadTemporaryDiary(initialTempDiaryId)
          }
        ]
      );
    }
  ...
  const startRecording = async () => {
    try {
      ...
      setRecordedURI(null);
      setContent(''); 
      setCurrentDiaryId(null); // 새 녹음 시작 시 기존 ID는 의미 없을 수 있음 (정책에 따라 결정)
    } catch (err) {
  ...
  const convertVoiceToText = async (voiceUri) => {
    ...
    try {
      console.log('--- 음성 변환 요청 ---'); // ... (기존 코드 동일)
      const response = await fetch(`${BASE_URL}/api/voice-to-text`, { /* ... */ });
      // ... (기존 코드 동일)
      if (!response.ok) { // ... (기존 코드 동일)
        throw new Error(errorMessage);
      }
      const result = await response.json();
      if (result && typeof result.text === 'string') { // ... (기존 코드 동일)
        return result.text;
      } else { // ... (기존 코드 동일)
